...
public class DataLogicPresenceManagement extends BeanFactoryDataSingle {
    ...
    private SentenceExec m_insertdatabase;
    ...
    public void init(Session s){
        ...
        tbreaks = new TableDefinition(s
            , "breaks"
            , new String[] { "ID", "NAME", "NOTES", "VISIBLE", "SITEGUID"}
            , new String[] { "ID", AppLocal.getIntString("label.epm.employee"), AppLocal.getIntString("label.epm.notes"), "VISIBLE","SITEGUID"}
            , new Datas[] { Datas.STRING, Datas.STRING, Datas.STRING, Datas.BOOLEAN, Datas.STRING}
            , new Formats[] { Formats.STRING, Formats.STRING, Formats.STRING, Formats.BOOLEAN, Formats.STRING}
            , new int[] {0}
        );
        ...
        m_startbreak =  new PreparedSentence(s
                , "INSERT INTO shift_breaks(ID, SHIFTID, BREAKID, STARTTIME,SITEGUID) VALUES (?, ?, ?, ?,?)"
                , new SerializerWriteBasic(new Datas[] {Datas.STRING, Datas.STRING, Datas.STRING, Datas.TIMESTAMP,Datas.STRING}));
...
//            , "SELECT COUNT(*) FROM shift_breaks WHERE ENDTIME IS NULL AND SHIFTID = ?"
        ...
        m_insertdatabase = new StaticSentence(s,
    ...
    public final SentenceList getBreaksList() {
        return new StaticSentence(s
            ...
            , null
            , (DataRead dr) -> new BreaksInfo(dr.getString(1), dr.getString(2)));
    }
 ...
 public Object readValues(DataRead dr) throws BasicException {
     ...
     * @throws BasicException
     */
    public final void StartBreak(String UserID,  String BreakID, String Siteguid) throws BasicException {
        String ShiftID = GetShiftID(UserID);
        String id = UUID.randomUUID().toString();
        Object[] value = new Object[] {id, ShiftID, BreakID, new Date(), Siteguid};
        m_startbreak.exec(value);
//        m_insertdatabase.exec(UUID.randomUUID().toString(),"I","shift_breaks",id,"id",ShiftID,"shiftid",Siteguid,0,0);
