...
public class JCatalog extends JPanel implements ListSelectionListener, CatalogSelector {
    ...
    private static final long serialVersionUID = 2327306009360397509L;

    private static final Logger LOG = Logger.getLogger(JCatalog.class.getName());
...
//        tnbcat = new ThumbNailBuilder(width, height, "com/openbravo/images/category.png");
    ...
    protected void fireSelectedProduct(ProductInfoExt prod) {
        ...
        LOG.log(Level.INFO, "Selected Product: {0}", prod);
        ...
        try {
            if (!m_categoriesset.contains(catid)) {
                ...
                for (ProductInfoExt prod : prods) {
                    loadProductStockAndStockAttributes(prod);
                    jcurrTab.addButton(
    ...
    private void loadProductStockAndStockAttributes(ProductInfoExt productInfo) throws BasicException {
        final List<AttributedProductStock> productStocks
                = m_dlSales.getAttributedProductStock().list(productInfo.getID());
        for (AttributedProductStock productStock : productStocks) {
            final List<AttributedProductStockAttribute> stockAttributes = m_dlSales
                    .getAttributedProductStockAttributes()
                    .list(productStock.getId());

            LOG.log(Level.INFO, "Product Stock: {0} = {1}",
                    new Object[]{productStock.getName(), stockAttributes.size()});

            productInfo.getProductStockAttributess().put(productStock, stockAttributes);
        }
    }
    ...
    private String getProductLabel(ProductInfoExt product) {
        ...
        if (pricevisible) {
            ...
            } else if (!"".equals(product.getDisplay())) {
            ...
            if (m_productsset.containsKey(id)) {
                ...
                try {
                    ...
                    if (products.isEmpty()) {
                        ...
                        loadProductStockAndStockAttributes(product);
