...
public class DataLogicSystem extends BeanFactoryDataSingle {
    ...
    protected SentenceExec m_siteguidSflag;
    
    protected SentenceExec m_siteguidLflag;
    
    public SentenceList n_lineremoved;
    
    public SentenceList m_sitects;
    
    protected SentenceExec mUpdatects;
...
// duplicate this for now as will extend in future release 
 ...
 // END OF CUSTOMER ******************************************************************      
        ...
        m_insertcash = new StaticSentence(s
                , "INSERT INTO closedcash(MONEY, HOST, HOSTSEQUENCE, DATESTART, DATEEND,SITEGUID) " +
                  "VALUES (?, ?, ?, ?, ?,?)"
                , new SerializerWriteBasic(new Datas[] {
                    Datas.STRING, 
                    Datas.STRING, 
        ...
        m_draweropened = new StaticSentence(s
                , "INSERT INTO draweropened ( NAME, TICKETID,SITEGUID) " +
                  "VALUES (?, ?,?)"
                , new SerializerWriteBasic(new Datas[] {
                    Datas.STRING, 
                    Datas.STRING, Datas.STRING}));       
        ...
        m_lineremoved = new StaticSentence(s,
                "INSERT INTO lineremoved (NAME, TICKETID, PRODUCTID, PRODUCTNAME, UNITS, SITEGUID) " +
                "VALUES (?, ?, ?, ?, ?,?)",
                new SerializerWriteBasic(new Datas[] {
                    Datas.STRING, Datas.STRING, 
                    Datas.STRING, Datas.STRING, 
        ...
        m_ticketremoved = new StaticSentence(s,
                "INSERT INTO lineremoved (NAME, TICKETID, PRODUCTNAME, UNITS , SITEGUID) " +
                "VALUES (?, ?, ?, ? , ?)",
                new SerializerWriteBasic(new Datas[] {
                    Datas.STRING, Datas.STRING,
                    Datas.STRING, Datas.DOUBLE, Datas.STRING                   
                }));  
         n_lineremoved = new StaticSentence(s,"SELECT max(autoid) from lineremoved",null, (DataRead dr) -> (                       
                dr.getInt(1)
                ));
...
//  Push Customers into CSVImport table      
        ...
        m_sitects = new StaticSentence(s,"Select cts from siteguid", null, 
                 (DataRead dr) -> (
                       dr.getInt(1)
                ));
        ...
        m_siteguidSflag = new StaticSentence(s,"update siteguid set sflag = ?",SerializerWriteInteger.INSTANCE);
        
        m_siteguidLflag = new StaticSentence(s,"update siteguid set siteguid = ?",SerializerWriteString.INSTANCE);
        
        
        mUpdatects = new StaticSentence(s,"update siteguid set cts = ?", SerializerWriteInteger.INSTANCE);
...
//    private final byte[] getResource(String name) {
    ...
    public String getlastLineremoved() throws BasicException{

        return  n_lineremoved.list(1).toString().replaceAll("\\[|\\]", "").replaceAll(", ", "\t");        
    }
    /**
     *
     * @param csv
...
// This is used by CSVimport to detect what type of product insert we are looking at, or what error occured
     ...
     */
       ...
       return "new";
        ...
        public final String getCts() throws BasicException{
            return m_sitects.list(1).toString().replaceAll("\\[|\\]", "").replaceAll(", ", "\t"); 
    ...
    */
    ...
    public void m_siteguidSflag(int sflag) throws BasicException {
        m_siteguidSflag.exec(sflag);
    }
    
    public void m_siteguidLflag(String lflag) throws BasicException {
        m_siteguidLflag.exec(lflag);
    }
    
    public void mUpdatects(int cts) throws BasicException {
        mUpdatects.exec(cts);
    }
}
