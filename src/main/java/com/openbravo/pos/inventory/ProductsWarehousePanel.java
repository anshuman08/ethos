    ...
    protected void init() {   
        ...
        lpr = new ListProviderCreator(new PreparedSentence(app.getSession(),
            "SELECT L.ID, P.ID, P.REFERENCE, P.NAME," +
                "L.STOCKSECURITY, L.STOCKMAXIMUM, COALESCE(S.SUMUNITS, 0) , P.SITEGUID " +
                "FROM products P " +
            ...
            public int execInTransaction(Object params) throws BasicException {
                ...
                if (values[0] == null)  {
                    ...
                    return new PreparedSentence(app.getSession()
                        , "INSERT INTO stocklevel (ID, LOCATION, PRODUCT, STOCKSECURITY, STOCKMAXIMUM, SITEGUID) VALUES (?, ?, ?, ?, ? , ?)"
                        , new SerializerWriteBasicExt(row.getDatas(), new int[] {0, 4, 1, 5, 6, 8})).exec(params);
                } else {
                    // UPDATE
                    return new PreparedSentence(app.getSession()
        ...
        jeditor = new ProductsWarehouseEditor(app,dirty);   
    }
