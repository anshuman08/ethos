...
public class JInventoryLines extends javax.swing.JPanel {
    ...
    public JInventoryLines() {
        ...
        c = new TableColumn(0, 150, leftAdjustedRenderer(), jtextFieldEditor());
        c.setHeaderValue(AppLocal.getIntString("label.item"));
        columns.addColumn(c);

        c = new TableColumn(1, 75, rightAdjustedRenderer(), jtextFieldEditor());
        c.setHeaderValue(AppLocal.getIntString("label.units2"));
        columns.addColumn(c);

        c = new TableColumn(2, 100, rightAdjustedRenderer(), jtextFieldEditor());
        c.setHeaderValue(AppLocal.getIntString("label.price"));
        columns.addColumn(c);

        c = new TableColumn(3, 100, rightAdjustedRenderer(), jtextFieldEditor());
        ...
        m_inventorylines = new InventoryTableModel(m_tableinventory);
        m_tableinventory.setModel(m_inventorylines);
        m_tableinventory.setFocusable(true);
        m_tableinventory.setRequestFocusEnabled(true);

    }

    private TableCellRenderer rightAdjustedRenderer() {
        return new DataCellRenderer(javax.swing.SwingConstants.RIGHT);
    }

    private TableCellRenderer leftAdjustedRenderer() {
        return new DataCellRenderer(javax.swing.SwingConstants.LEFT);
    }

    private TableCellEditor jtextFieldEditor() {
    ...
    private static class InventoryTableModel extends AbstractTableModel {
        ...
        private static final long serialVersionUID = -8407923797613791648L;

        private final ArrayList<InventoryLine> m_rows = new ArrayList<>();

        private final JTable inventoryTable;

        public InventoryTableModel(JTable inventoryTable) {
        ...
        public Object getValueAt(int row, int column) {
            final InventoryLine i = m_rows.get(row);
            switch (column) {
                case 0:
        ...
        public void setValueAt(final Object aValue, final int rowIndex, final int columnIndex) {
            final InventoryLine i = m_rows.get(rowIndex);
            if (aValue == null) {
                return;
            }
            ...
            switch (columnIndex) {
                ...
                case 3:
                    break;
                case 1:
                    final String unitsX = aValue.toString();
                    final String strippedOfX = unitsX.startsWith("x") ? unitsX.substring(1) : unitsX;
                    try {
                        i.setMultiply(Double.parseDouble(strippedOfX));
                    } catch (final NumberFormatException nfe) {
                        showMessageDialog(inventoryTable, "Invalid Value for units, please use only numbers");
                    }
                    break;
                case 2:
                    try {
                        final String currValue = aValue.toString();
                        final double withoutCurr = (double) Formats.CURRENCY.parseValue(currValue);
                        i.setPrice(withoutCurr);
                    } catch (final BasicException be) {
                        showMessageDialog(inventoryTable, "Invalid Value for price, please use only decimals");
                    }
                    break;
            }
