...
//        m_dlSales.insertDatabaseChangeLog(new Object[]{UUID.randomUUID().toString(),m_process,"stockdiary",m_sID,"id",productid,"product",g_siteguid,0,0});
        return new Object[]{
            m_sID, //1
            Formats.TIMESTAMP.parseValue(m_jdate.getText()), //2
            m_ReasonModel.getSelectedKey(), //3
            m_LocationsModel.getSelectedKey(), //4
            productid, //5
            attsetinstid, //6
            samesignum((Double) Formats.DOUBLE.parseValue(m_junits.getText()), (Integer) m_ReasonModel.getSelectedKey()), //7
            Formats.CURRENCY.parseValue(m_jprice.getText()), //8
            m_App.getAppUserView().getUser().getName(), //9
            productref, //10
            productcode, //11
            productname, //12
            attsetid, //13
            attsetinstdesc, //14
            g_siteguid, //    15        m_SuppliersModel.getSelectedKey()
        };
    }
