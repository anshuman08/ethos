...
public final class TicketInfo implements SerializableRead, Externalizable {
    ...
    public static final int RECEIPT_APP = 4;
...
//    private SiteguidInfo m_siteguid;
    ...
    public TicketInfo() {
        m_sId = UUID.randomUUID().toString()+UUID.randomUUID().toString().hashCode();
...
//        m_siteguid = (SiteguidInfo) in.readObject();
    }
...
//    public SiteguidInfo getSiteguid(){
//        return m_siteguid;
//    }
//    public String getSiteguId(){
//        return m_siteguid.getId();
//    }
...
//    }
    ...
    public double getDisSubValue(){
        return getTotalDiscount() + getSubTotal();
    }
    ...
    public double getTotalDiscount() {
        ...
        double orPrice = 0.0;
        TicketLineInfo oLine = null;
        for (Iterator<TicketLineInfo> i = m_aLines.iterator(); i.hasNext();) {
            oLine = i.next();
            orPrice = orPrice + (oLine.getMultiply()*oLine.m_productprice);
            
             System.out.println("Total price ************** : " + orPrice);
        }
       System.out.println("Original  Total Value ************** : " + getSubTotal());
      
       return orPrice - getSubTotal();
    }
...
//    public double getTotalDiscount() {
//        Optional<Double> totalDiscount = m_aLines.stream().map(TicketLineInfo::getDiscount).reduce(Double::sum);
//        return totalDiscount.isPresent() ? totalDiscount.get() : 0.0;
//    }
    ...
    public double getOrigTotal() {
        ...
        for (Iterator<TicketLineInfo> i = m_aLines.iterator(); i.hasNext();) {
            oLine = i.next();
            orTotal = orTotal + (oLine.getMultiply()*oLine.m_productprice);
        }
...
// Added JDL 28.05.13 for loyalty card functions
    ...
    public String printDisSubvalue(){
        return Formats.CURRENCY.formatValue(getDisSubValue());
    }
    ...
    public String printDiscount() {
        return Formats.CURRENCY.formatValue(getTotalDiscount());
    }
    ...
    public String printOrigTotal() {
        return Formats.CURRENCY.formatValue(getOrigTotal());
    }
