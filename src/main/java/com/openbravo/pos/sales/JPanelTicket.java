...
import com.openbravo.pos.inventory.AttrValueInfo;
import com.openbravo.pos.inventory.AttributedProductStock;
import com.openbravo.pos.inventory.AttributedProductStockAttribute;
...
import com.openbravo.pos.util.autocomplete.AutoCompleteJComboBox;
...
import static com.openbravo.pos.sales.JAttributedProductStockSelection.showSelectAttributedStock;
...
import com.openbravo.pos.util.autocomplete.BoundsPopupMenuListener;
...
public abstract class JPanelTicket extends JPanel implements JPanelView, BeanFactoryApp, TicketsEditor {
    ...
    private static final Logger LOG = Logger.getLogger(JPanelTicket.class.getName());
    ...
    protected MobileAppInfo mObileAppInfo;

    protected DatalogicMobileAppSales m_dMobileApp;
    ...
    public void init(AppView app) throws BeanFactoryException {
        ...
        m_dMobileApp = (DatalogicMobileAppSales) app.getBean("com.openbravo.pos.sales.mobileorders.DatalogicMobileAppSales");

        /*
         * uniCenta Feb 2017 Changed for 4.3 Set up main toolbar area with two rows to add cater for additional scripts
         * else over-crowding and some dynamic buttons off screen/not visible m_jPanelScripts contains m_jButtonsExt
         * panel for buttons Enabled/Disabled in Resources>Menu.Root
         */
...
// Configuration>Peripheral options
        ...
        //enable product selection drop down.
        try {
            //set the item selection
            final List<ProductInfoExt> productInfo = dlSales.getExtendedProductsList().list();
            final AutoCompleteJComboBox selectionBox = (AutoCompleteJComboBox) productSelectionBox;
            selectionBox.setProductInfoExt(productInfo);
            
            BoundsPopupMenuListener listener =  new BoundsPopupMenuListener(true, false);
            selectionBox.addPopupMenuListener( listener );
            selectionBox.setPrototypeDisplayValue("ItemWWW");
...
//                frame.setVisible(true);
            ...
            productSelectionBox.addItemListener((ev) -> {
                if (selectionBox.isEditing() || ev.getStateChange() != ItemEvent.SELECTED) {
                    return;
                }
                final ProductInfoExt item = selectionBox.getSelectedProductInfoExt();
                if (item != null) {
                    addTicketLine(item, getPorValue(), item.getPriceSell());
                }
    ...
    public void setActiveTicket(TicketInfo oTicket, Object oTicketExt) {
        ...
        if (oTicket != null && oTicket.getTicketType() == 4) {
            try {
                mObileAppInfo = m_dMobileApp.getOrder(oTicket.getTicketId());
            } catch (BasicException ex) {
                Logger.getLogger(JPanelTicket.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
...
//            repaint();
    ...
    public void addTicketLine(ProductInfoExt oProduct, double dMul, double dPrice) {
        if (m_oTicket.getTicketType() == 4) {
            JOptionPane.showMessageDialog(this, "This is App's Delivery Order Items cannot be added kindly");
        } else {
            if (oProduct.isVprice()) {
                ...
                addTicketLine(new TicketLineInfo(oProduct, dMul, dPrice, tax,
                        (java.util.Properties) (oProduct.getProperties().clone()), g_siteguid));
...
// get the line product tax
        ...
        if (executeEventAndRefresh("ticket.addline", new ScriptArg("line", oLine)) == null) {
            if (oLine.isProductCom()) {
                ...
                try {
                    ...
                    LOG.log(Level.INFO, "Checking for attributed stocks for product: {0}", oLine.getProductName());

                    final List<AttributedProductStockSelection> stockSelections
                            = showSelectAttributedStock(dlSales, oLine.getProductID());
                    if (!stockSelections.isEmpty()) {
                        //Maintain the product bundle price.
                        final List<String> productStockIds = stockSelections.stream()
                                .map(AttributedProductStockSelection::getProductStock)
                                .map(AttributedProductStock::getId)
                                .collect(toList());
                        oLine.setProductStockIds(productStockIds);

                        //How do we handle attributesetinstanceid?
                        final Map<String, Integer> prodStockIds = stockSelections.stream()
                                .map((selection) -> {
                                    final AttributedProductStock selectedProductStock = selection.getProductStock();
                                    final int qtyRatio = selection.getQuantityInBunlde();
                                    changeProductStockQty(1, qtyRatio, selectedProductStock.getId());

                                    return Pair.of(selectedProductStock.getId(), qtyRatio);
                                })
                                .collect(toMap(Pair::getKey, Pair::getValue));

                        final Map<String, String> attrSetInstanceIds = stockSelections.stream()
                                .map(AttributedProductStockSelection::getProductStock)
                                .map(this::addCompatibleAttributeSetInstance)
                                .filter(Objects::nonNull)
                                .collect(toMap(Pair::getKey, Pair::getValue));
                        oLine.setProductAttrSetInstIds(new ArrayList<>(attrSetInstanceIds.values()));

                        // Add mapping for attrinstsetid to qty for the ticket line
                        attrSetInstanceIds.forEach((prodStockId, attrSetIns) -> {
                            final int qtyRatio = prodStockIds.getOrDefault(prodStockId, 0);

                            LOG.log(Level.INFO,
                                    "Adding attr-inst-id qty-ratio: ({0}, {1})",
                                    new Object[]{attrSetIns, qtyRatio});

                            oLine.addProductAttrSetInstanceIdQtyRatio(attrSetIns, qtyRatio);

                            //add attr product stock ref
                            oLine.addProductAttrSetInstanceProductStockId(attrSetIns, prodStockId);
                        });

                        final String attrSetInstanceDesc = stockSelections.stream()
                                .map(AttributedProductStockSelection::getProductStock)
                                .sorted(comparing(AttributedProductStock::getName))
                                .map(AttributedProductStock::getName)
                                .collect(joining(","));
                        oLine.setProductAttSetInstDesc(attrSetInstanceDesc);
                    } else {
                        final List<AttributedProductStock> productStocks = dlSales
                                .getAttributedProductStock()
                                .list(oLine.getProductID());
                        final AttributedProductStock selectedProductStock;
                        if (!productStocks.isEmpty() && (selectedProductStock = (AttributedProductStock) JOptionPane
                                .showInputDialog(
                                        this,
                                        "There are several product stock for this product",
                                        "Select Product Stock",
                                        JOptionPane.OK_OPTION,
                                        null,
                                        productStocks.toArray(),
                                        null)) != null) {
                            oLine.setProductStockId(selectedProductStock.getId());
                            oLine.setPrice(selectedProductStock.getPrice());

                            //Product stock selected, we need to increase order qty
                            changeProductStockQty(1, 1, selectedProductStock.getId());
                            final Pair<String, String> attrSetInstanceId
                                    = addCompatibleAttributeSetInstance(selectedProductStock);

                            LOG.log(Level.INFO, "Setting attrSetInstanceId: {0}", attrSetInstanceId);

                            if (attrSetInstanceId != null) {
                                oLine.setProductAttSetInstId(attrSetInstanceId.getValue());
                            }

                            oLine.setProductAttSetInstDesc(selectedProductStock.getName());
                        } else if (oLine.isProductVerpatrib()) {
                    ...
                    if (!m_ticketlines.isTicketLineAdded(oLine)) {
                        m_oTicket.addLine(oLine);
                        m_ticketlines.addTicketLine(oLine);

                        final int i = m_ticketlines.getSelectedIndex();
                        paintTicketLine(i, oLine);
                    } else {
                        final int i = m_ticketlines.getTicketLineInfoIndex(oLine);
    ...
    private Pair<String, String> addCompatibleAttributeSetInstance(final AttributedProductStock productStock) {
        try {
            final String attrSetId = productStock.getAttributeSetId();

            final List<AttributedProductStockAttribute> stockAttributes = dlSales.getAttributedProductStockAttributes()
                    .list(productStock.getId());
            final String description = stockAttributes.stream()
                    .map(AttributedProductStockAttribute::getAttrValueInfo)
                    .map(AttrValueInfo::getValue)
                    .collect(joining(", "));
            final String attrSetInstanceId = (String) dlSales.findAttributeSetInstanceId()
                    .find(attrSetId, description);
            if (attrSetInstanceId != null) {
                //Already added, continue
                return Pair.of(productStock.getId(), attrSetInstanceId);
            }

            //Create new instance
            final String newAttrSetInstanceId = UUID.randomUUID().toString();
            dlSales.addAttributeSetInstance()
                    .exec(newAttrSetInstanceId, attrSetId, description, g_siteguid);
            dlSales.insertDatabaseChangeLog2()
                    .exec(UUID.randomUUID().toString(), "I", "attributesetinstance", newAttrSetInstanceId, "id", attrSetId, "attributeset_id", g_siteguid, 0, 0);
            //Add attr instance from the product stock attributes
            stockAttributes.forEach((stockAttr) -> {
                try {
                    final String attrInstanceId = UUID.randomUUID().toString();
                    final AttrValueInfo attrValueInfo = stockAttr.getAttrValueInfo();
                    final String attrId = attrValueInfo.getAttributeId();
                    final String attrValue = attrValueInfo.getValue();
                    dlSales.addAttributeInstance()
                            .exec(attrInstanceId, newAttrSetInstanceId, attrId, attrValue, g_siteguid);
                    dlSales.insertDatabaseChangeLog2()
                            .exec(UUID.randomUUID().toString(), "I", "attributeinstance", attrInstanceId, "id", newAttrSetInstanceId, "attributesetinstance_id", g_siteguid, 0, 0);
                } catch (final BasicException ex) {
                    Logger.getLogger(JPanelTicket.class.getName()).log(Level.SEVERE, null, ex);
                }
            });

            return Pair.of(productStock.getId(), newAttrSetInstanceId);
        } catch (BasicException ex) {
        ...
        if (executeEventAndRefresh("ticket.removeline", new ScriptArg("index", i)) == null) {
            ...
            dlSystem.execLineRemoved(
                    new Object[]{
                        ...
                        m_oTicket.getLine(i).getMultiply(), g_siteguid
                    });
            try {
                if (dlSystem.getlastLineremoved() != null) {
            ...
            final TicketLineInfo ticketLineInfo = m_oTicket.getLine(i);
            m_ticketlines.removeTicketLineInfo(ticketLineInfo);

            if (ticketLineInfo != null) {
                final int changeQty = (int) -ticketLineInfo.getMultiply();
                updateOrderQuantity(ticketLineInfo, changeQty);
            }
            ...
            if (m_oTicket.getLine(i).isProductCom()) {
                ...
                if (m_App.getAppUserView().getUser().hasPermission("sales.DeleteLines")) {
                    int input = JOptionPane.showConfirmDialog(this,
                            AppLocal.getIntString("message.deletelineyes"), AppLocal
                            .getIntString("label.deleteline"),
                    ...
                    JOptionPane.showMessageDialog(this,
                            AppLocal.getIntString("message.deletelineno"), AppLocal.getIntString(
                            "label.deleteline"), JOptionPane.WARNING_MESSAGE);
                }
            } else {
                m_oTicket.removeLine(i);
                m_ticketlines.removeTicketLine(i);
    ...
    private void updateOrderQuantity(final TicketLineInfo ticketLineInfo, final int changeQty) {
        //if this is an attributed stock, increase order count.
        try {
            final String attrStockId = ticketLineInfo.getProductStockId();
            if (attrStockId != null) {
                final AttributedProductStock productStock = (AttributedProductStock) dlSales
                        .getAttributedProductStockById()
                        .find(attrStockId);
                if (productStock != null) {
                    changeProductStockQty(changeQty, 1, attrStockId);
                }
            } else {
                final List<String> prodStockIds = ticketLineInfo.getProductStockIds();
                final String productId = ticketLineInfo.getProductID();
                final Map<String, Integer> productStockBundleQty = getProductStockBundleQuantity(productId);
                prodStockIds.stream()
                        .filter(productStockBundleQty::containsKey)
                        .forEach((stockId) -> {
                            final int qtyRatio = productStockBundleQty.getOrDefault(stockId, 0);
                            changeProductStockQty(changeQty, qtyRatio, stockId);
                        });
            }
        } catch (BasicException ex) {
            LOG.log(Level.SEVERE, null, ex);
    ...
    private void changeProductStockQty(final int changeQty, final int qtyRatio, final String prodStockId) {
        final int orderQtyChange = changeQty * qtyRatio;
        final int stockQtyChange = -changeQty * qtyRatio;

        LOG.log(Level.INFO, "<{0}> Changing order qty by: {1}", new Object[]{prodStockId, orderQtyChange});

        LOG.log(Level.INFO, "<{0}> Changing stock qty by: {1}", new Object[]{prodStockId, stockQtyChange});

        try {
            dlSales.changeAttributeProductStockOrderQty()
                    .exec(orderQtyChange, stockQtyChange, prodStockId);
        } catch (BasicException ex) {
            LOG.log(Level.SEVERE, null, ex);
        }
    }

    private Map<String, Integer> getProductStockBundleQuantity(final String productId) throws BasicException {
        ...
        //Configure selection
        final Map<String, Double> bundleQtyByProductId = productsBundleInfos.stream()
        ...
        final List<AttributedProductStock> attributedProductStocks = dlSales
                .getAttributedProductStockByProductIds(bundleQtyByProductId.keySet());
        final Map<String, Integer> productStockQuantities = new HashMap<>();
        attributedProductStocks.stream()
                .forEach((prodStock) -> {
                    final String stockProductId = prodStock.getProductId();
                    final int qty = bundleQtyByProductId.getOrDefault(stockProductId, 0.0).intValue();
                    productStockQuantities.put(prodStock.getId(), qty);
                });

        return productStockQuantities;
    }
        ...
        if ((cTrans == '\n') || (cTrans == '?')) {
            ...
            if (m_sBarcode.length() > 0) {
                ...
                if (sCode.startsWith("C")
                    ...
                    /*
                     * This block is deliberately verbose and is base for future scanner handling Some scanners inject a
                     * CR+LF... some don't... stateTransition() must allow for this as these add characters to .length()
                     * First 3 digits are GS1 CountryCode OR Retailer internal use
                     *
                     * Prefix ManCodeProdCode CheckCode PPP MMMMMCCCCC K 012 3456789012 K Barcode CCCCC must be unique
                     * Notes: ManufacturerCode and ProductCode must be exactly 10 digits If code begins with 0 then is
                     * actually a UPC-A with prepended 0
                     *
                     * uniCenta oPOS Retailer instore uses these RULES Prefixes 020 to 029 are set aside for Retailer
                     * internal use This means that CCCC becomes price/weight values Prefixes 978 and 979 are set aside
                     * for ISBN - Future use
                     *
                     * Prefix ManCode ProdCode CheckCode PPP MMMMM CCCCC K Format 012 34567 89012 K Human
                     *
                     */
...
//  WEIGHT - SET value decimals
                            ...
                            TaxInfo tax = taxeslogic // get the TaxRate for the product
                                    .getTaxInfo(oProduct.getTaxCategoryID(), m_oTicket.getCustomer());                         // calculate if ticket has a Customer
...
//  PRICE - Assign var's
                                ...
                                case "20":                                          // as above
                                    ...
                                    oProduct
                                            .setProperty("product.price", Double.toString(oProduct.getPriceSellTax(tax)));
                                    break;
                                case "21":
                                    dPriceSell = oProduct.getPriceSellTax(tax)
                                            / (1.0 + tax.getRate());
...
// WEIGHT - Assign variable to Unit
                                ...
                                /*
                                 * Some countries use different barcode prefix 26-29 or 250 etc. Use this section to add
                                 * more case statements but these are not mandatory If you have your own internal or
                                 * other barcode schema then... Example: case "28": { // price has tax. Remove it from
                                 * sPriceSell TaxInfo tax = taxeslogic.getTaxInfo(oProduct.getTaxCategoryID(),
                                 * m_oTicket.getCustomer()); dPriceSell /= (1.0 + tax.getRate());
                                 * oProduct.setProperty("product.price", Double.toString(dPriceSell)); weight = -1.0;
                                 * break;
                                 */
                                default:
                                    break;
                            ...
                            if (m_jaddtax.isSelected()) {
                                //weight
                                addTicketLine(oProduct, dUnits, dPriceSell = oProduct.getPriceSellTax(tax));
                            } else {
...
// UPC-A
/*
                     * Note: if begins 02 then its a standard // UPC-A max value limitation is 4 digit price // UPC-A
                     * Extended uses State digit to give 5 digit price // uniCenta oPOS does not support UPC-A Extended
                     * at this time // Identifier Prod State Cost CheckCode // I PPPPP S CCCC K // 1 23456 7 8901 2
                     *
                     * 0 = Standard UPC number (must have a zero to do zero-suppressed numbers) 1 = Reserved 2 =
                     * Random-weight items (fruits, vegetables, meats, etc.) 3 = Pharmaceuticals 4 = In-store marketing
                     * for retailers (Other stores will not understand) 5 = Coupons 6 = Standard UPC number 7 = Standard
                     * UPC number 8 = Reserved 9 = Reserved
                     */
...
// END OF BARCODE
                        ...
                        if (autoLogoff != null) {
                            if (autoLogoff.equals("true")) {
                                if ("restaurant".equals(
                                        ...
                                        && ("true"
                                                .equals(m_App.getProperties().getProperty("till.autoLogoffrestaurant")))) {
                                    deactivate();
                                    setActiveTicket(null, null);
                                } else {
                                    ((JRootApp) m_App).closeAppView();
            ...
            try {
                ...
                if (executeEvent(ticket, ticketext, "ticket.total") == null) {
                    ...
                    if (ticket.getTicketType() == 4) {
                        paymentdialog = ticket.getTicketType() == TicketInfo.RECEIPT_APP
                                ? paymentdialogreceipt
                                : paymentdialogrefund;
                    } else {
                        paymentdialog = ticket.getTicketType() == TicketInfo.RECEIPT_NORMAL
                                ? paymentdialogreceipt
                                : paymentdialogrefund;
                    }
                        ...
                        if (executeEvent(ticket, ticketext, "ticket.save") == null) {
                            if (ticket.getTicketType() != 4) {
                                try {
                                    ...
                                    MessageInf msg = new MessageInf(MessageInf.SGN_NOTICE, AppLocal.getIntString(
                                            "message.nosaveticket"), eData);
                                    msg.show(this);
                                 ...
                                 try {
                                  dlSales.saveTicket(ticket, m_App.getInventoryLocation());
                                  mObileAppInfo.setTransaction_id(ticket.getTransactionID());
                                  updateAppOrder(ticket);
                                 } catch (BasicException eData) {
                                    MessageInf msg = new MessageInf(MessageInf.SGN_NOTICE, AppLocal.getIntString(
                                            "message.nosaveticket"), eData);
                                    msg.show(this);
    ...
    public void updateAppOrder(TicketInfo ticket){
        try{
            mObileAppInfo.setOrder_status("confirmed");
            ...
            json.put("orderStatus", mObileAppInfo.getOrder_status());
            ...
            if (obj.get("status").toString().equals("1")) {
               m_dMobileApp.setOrderUpdate(mObileAppInfo);
            }
            conn.disconnect();
    ...
    private void printTicket(String sresourcename, TicketInfo ticket, Object ticketext) {
        ...
        if (sresource == null) {
            ...
            try {
                ...
                script.put("mobileorder",mObileAppInfo);
                refreshTicket();
...
//        clock.setTimePattern ( clock );
    ...
    private void printReport(String resourcefile, TicketInfo ticket, Object ticketext) {
        ...
        try {
            ...
            JasperPrint jp = JasperFillManager.fillReport(jr, reportparams, new JRMapArrayDataSource(
                    new Object[]{reportfields}));
    ...
    private void visorTicketLine(TicketLineInfo oLine) {
        if (oLine == null) {
            ...
            try {
                ...
                MessageInf msg = new MessageInf(MessageInf.SGN_WARNING, AppLocal.getIntString("message.cannotprintline"),
                        e);
                msg.show(JPanelTicket.this);
            }
        }
...
//                Toolkit.getDefaultToolkit().beep();
            ...
            try {
                ...
                if (line.getProductStockId() != null) {
                    final AttributedProductStock attrProductStock
                            = dlSales.findAttributedProductStock(line.getProductStockId());
                    if (attrProductStock != null) {
                        if (attrProductStock.getQuantity() <= 0) {
                            jCheckStock.setForeground(Color.magenta);
                        } else {
                            jCheckStock.setForeground(Color.darkGray);
                        }
                        jCheckStock.setText(Integer.toString(attrProductStock.getQuantity()));
                        return;
                    } else {
                        jCheckStock.setText(null);
                    }
    ...
    private void initComponents() {
        ...
        productSelectionBox = new com.openbravo.pos.util.autocomplete.AutoCompleteJComboBox();
        ...
        m_jPanelBag.setMinimumSize(new java.awt.Dimension(350, 54));
        m_jPanelBag.setPreferredSize(new java.awt.Dimension(0, 60));

        productSelectionBox.setEditable(true);
        productSelectionBox.setFont(new java.awt.Font("Arial", 1, 20)); // NOI18N
        productSelectionBox.setForeground(new java.awt.Color(120, 115, 115));
        productSelectionBox.setBorder(new javax.swing.border.LineBorder(new java.awt.Color(213, 210, 210), 1, true));
        productSelectionBox.setMaximumSize(new java.awt.Dimension(170, 45));
        productSelectionBox.setMinimumSize(new java.awt.Dimension(90, 30));
        productSelectionBox.setPreferredSize(new java.awt.Dimension(170, 45));
        productSelectionBox.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                productSelectionBoxActionPerformed(evt);
            }
        });
        m_jPanelBag.add(productSelectionBox);
    ...
    private void jEditAttributesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jEditAttributesActionPerformed
        ...
        if (m_oTicket.getTicketType() != 4) {
            ...
            JOptionPane.showMessageDialog(this,
                    AppLocal.getIntString("message.appordersnotallowed"),
                    AppLocal.getIntString("message.title"),
                    JOptionPane.INFORMATION_MESSAGE);
        }
    ...
    private void webCBCustomerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_webCBCustomerActionPerformed
            ...
            if (m_oTicket.getCustomerId() == null) {
                ...
                if (finder.getSelectedCustomer() != null) {
                    try {
                        ...
                        if ("restaurant".equals(m_App.getProperties().getProperty("machine.ticketsbag"))) {
                            restDB.setCustomerNameInTableByTicketId(dlSales.loadCustomerExt(finder.getSelectedCustomer()
                                .getId()).toString(), m_oTicket.getId());
        ...
        } else if (webCBCustomer.getSelectedIndex() == 1) {
    ...
    private void btnReprint1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnReprint1ActionPerformed
        if (m_config.getProperty("lastticket.number") != null) {
            try {
                ...
                MessageInf msg = new MessageInf(MessageInf.SGN_WARNING, AppLocal
                    .getIntString("message.cannotloadticket"), e);
                msg.show(this);
            }
        }
            ...
            if (((mooring.getVesselDays() > 0)) && ((mooring.getVesselSize() > 1))) {
                try {
                    ...
                    if (mooring.getVesselDays() > 1) {
                        ...
                        addTicketLine(vProduct, mooring.getVesselSize() * (mooring.getVesselDays() - 1), vProduct
                            .getPriceSell());
                    }
    ...
    private void productSelectionBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_productSelectionBoxActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_productSelectionBoxActionPerformed
    ...
    private javax.swing.JComboBox<String> productSelectionBox;
    private com.alee.laf.combobox.WebComboBox webCBCustomer;
    // End of variables declaration//GEN-END:variables

    /*
     * Remote Orders Display We only know about uniCenta oPOS orders and won't try and handle any that are injected from
     * an external source
     */
    ...
    public void remoteOrderDisplay(String id, Integer display, boolean primary) {
        ...
        for (int i = 0; i < m_oTicket.getLinesCount(); i++) {
            try {
                ...
                dlSystem.addOrder(getPickupString(m_oTicket), (int) m_oTicket.getLine(i).getMultiply(), m_oTicket
                        .getLine(i).getProductName(), m_oTicket.getLine(i).getProductAttSetInstDesc(),
                        m_oTicket.getLine(i).getProperty("notes"), id, null, display, null, null);

                /*
                 * this block for future - right now we're deleting all ticketlines and resending for consistency with
                 * actual ticketlines dlSystem.updateOrder(getPickupString(m_oTicket) , (int)
                 * m_oTicket.getLine(i).getMultiply() , m_oTicket.getLine(i).getProductName() ,
                 * m_oTicket.getLine(i).getProductAttSetInstDesc() , m_oTicket.getLine(i).getProperty("notes") , id ,
                 * null , display , null , null);
                 */
            } catch (BasicException ex) {
                Logger.getLogger(JPanelTicket.class.getName()).log(Level.SEVERE, null, ex);
            }
